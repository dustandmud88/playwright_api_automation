...
@when(parsers.parse('I send "{http_method}" request without body'))
def send_post_request(request_context, api_client, http_method, datatable=None):
    ...
    if datatable is not None:
        ...
        for column in range(len(datatable[1:])):
            if datatable[0][column] not in temporal_dict:
                temporal_dict[datatable[0][column]] = datatable[1][column]

    request_context["body"] = temporal_dict
